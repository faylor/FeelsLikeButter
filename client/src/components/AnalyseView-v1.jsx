  ...
  return (
    <div style={{ padding: "0 24px" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        {[
          ["01", "Preview extracted",   "on your device"],
          ["02", "Faces auto-blurred",  "browser AI, no upload"],
          ["03", "You draw extra zones","full manual control"],
          ["04", "AI sees only frames", "video never uploaded"],
        ].map(([n, t, s]) => (
...
// ─── AnalyzeView ──────────────────────────────────────────────────────────────
export function AnalyzeView({ stroke, setStroke, videoFile, setVideoFile, step, setStep, result, error, note, setNote, onPrivacyConfirm, onSave }) {
  const sc = T.strokes[stroke];

  if (step === "privacy") {
    return <PrivacyEditor videoFile={videoFile} onConfirm={onPrivacyConfirm} onBack={() => setStep("upload")} accent={sc.accent} />;
  }
